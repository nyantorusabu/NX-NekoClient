(function () {
	...
	async function fetchUrlInfo(url) {
		try {
			const infoUrl =
				...
				encodeURIComponent(PROXY_INFO_BASE + url);
	...
	function getPostContentText(postEl) {
		...
		const contentEl = postEl.querySelector(
			'.post-content, .notification-item-content, .user-me',
		);
		if (!contentEl) return '';
	...
	async function processPostElement(postEl) {
		...
		try {
			...
			const existing = Array.from(
				container.querySelectorAll('.link-preview'),
			).some((el) => {
	...
	function createObserverFor(container) {
		...
		container
			...
			.forEach((p) => processPostElement(p));
	}
